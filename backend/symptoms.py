...
# Define symptom dictionary and disease list
...
diseases_list = {15: 'Fungal infection', 4: 'Allergy', 16: 'GERD', 9: 'Chronic cholestasis', 14: 'Drug Reaction', 33: 'Peptic ulcer disease', 1: 'AIDS', 12: 'Diabetes', 17: 'Gastroenteritis', 6: 'Bronchial Asthma', 23: 'Hypertension', 30: 'Migraine', 7: 'Cervical spondylosis', 32: 'Paralysis (brain hemorrhage)', 28: 'Jaundice', 29: 'Malaria', 8: 'Chicken pox', 11: 'Dengue', 37: 'Typhoid', 40: 'Hepatitis A', 19: 'Hepatitis B', 20: 'Hepatitis C', 21: 'Hepatitis D', 22: 'Hepatitis E', 3: 'Alcoholic hepatitis', 36: 'Tuberculosis', 10: 'Common Cold', 34: 'Pneumonia', 13: 'Dimorphic hemorrhoids (piles)', 18: 'Heart attack', 39: 'Varicose veins', 26: 'Hypothyroidism', 24: 'Hyperthyroidism', 25: 'Hypoglycemia', 31: 'Osteoarthritis', 5: 'Arthritis', 0: 'Vertigo', 2: 'Acne', 38: 'Urinary tract infection', 35: 'Psoriasis', 27: 'Impetigo'}

# Load databases
sym_des = pd.read_csv(r"C:\Akshay\AI-MedLab-main\backend\HealthPredict\symtoms_df.csv")
precautions = pd.read_csv(r"C:\Akshay\AI-MedLab-main\backend\HealthPredict\precautions_df.csv")
workout = pd.read_csv(r"C:\Akshay\AI-MedLab-main\backend\HealthPredict\workout_df.csv")
description = pd.read_csv(r"C:\Akshay\AI-MedLab-main\backend\HealthPredict\description.csv")
medications = pd.read_csv(r"C:\Akshay\AI-MedLab-main\backend\HealthPredict\medications.csv")
diets = pd.read_csv(r"C:\Akshay\AI-MedLab-main\backend\HealthPredict\diets.csv")

# Load the model
model_path = r"C:\Akshay\AI-MedLab-main\backend\aimodels\svc.pkl"
try:
...
def get_predicted_value(symptoms):
    print(symptoms)  # Debug print
    # Convert symptoms to model input format
    sym_list = np.zeros(len(symptoms_dict))
    for sym in symptoms:
        if sym in symptoms_dict:
            sym_list[symptoms_dict[sym]] = 1
    sym_list = sym_list.reshape(1, -1)
    
    # Predict the disease
    try:
        prediction = model.predict(sym_list)
...
def helper(dis):
    ...
    pre = precautions[precautions['Disease'] == dis][['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']].dropna().values.flatten().tolist()
    med = medications[medications['Disease'] == dis]['Medication'].dropna().values.flatten().tolist()
    die = diets[diets['Disease'] == dis]['Diet'].dropna().values.flatten().tolist()
    wrkout = workout[workout['disease'] == dis]['workout'].dropna().values.flatten().tolist()
...
#     return diseases_list.get(model.predict([input_vector])[0], "Unknown disease")
...
#     parser.add_argument('data', type=str, help='Input data in JSON format')
#     parser.add_argument('--loads', type=str, required=True, help='Path to the model')
